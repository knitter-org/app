...
export class ChannelViewStore {
  readonly isLoading$ = store.pipe(select((state) => state.isLoading));

  readonly entries$ = store.pipe(selectAllEntities());
  ...
  async setChannel(channelId: string): Promise<void> {
    ...
    const entries = await this.channelService.unreadEntiresOrderedByDate(
      channelId
    );
    store.update(setEntities(entries), (state) => ({
      ...state,
      isLoading: false,
    }));
  }
  ...
  async markEntryAsRead(entry: Entry) {
    const updatedEntry = await this.feedService.markEntryAsRead(
      entry.feedId,
      entry.id,
      !entry.readAt
    );
